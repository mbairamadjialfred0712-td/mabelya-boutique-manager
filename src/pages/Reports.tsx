...
export default function Reports() {
  ...
  const { data: boutiques } = useQuery({
    queryKey: ["boutiques"],
    queryFn: async () => { const { data } = await supabase.from("boutiques").select("*, countries(name)").order("name"); return data ?? []; },
  });
  ...
  const { data: topProductsData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("sale_items")
        .select("product_id, quantity, total_price, products(name, boutique_id, boutiques(name, country_id, countries(name)))")
        .gte("created_at", dateFrom)
        .lte("created_at", dateTo + "T23:59:59");
      if (!data) return [];
  ...
  filteredSales?.forEach((s) => { const day = s.created_at.split("T")[0]; byDay[day] = (byDay[day] ?? 0) + Number(s.total_amount); });
  const dailyData = Object.entries(byDay).sort(([a], [b]) => a.localeCompare(b)).map(([date, revenue]) => ({
    date: new Date(date).toLocaleDateString("fr-FR", { day: "2-digit", month: "short" }), revenue,
  }));

  // Group by boutique
  const byBoutique: Record<string, number> = {};
  filteredSales?.forEach((s) => { const b = (s.boutiques as any)?.name ?? "Autre"; byBoutique[b] = (byBoutique[b] ?? 0) + Number(s.total_amount); });
  const boutiqueData = Object.entries(byBoutique).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);

  // Group by user (vendeur)
  ...
  const exportPDF = () => {
    ...
    (doc as any).autoTable({
      ...
      styles: { fontSize: 8 }, headStyles: { fillColor: [50, 80, 200] },
    });
    ...
    (doc as any).autoTable({
      startY: (doc as any).lastAutoTable.finalY + 10,
      head: [["Pays", "Ventes", "CA"]],
      body: countryData.map((c) => [c.name, c.sales, formatCurrency(c.revenue)]),
      styles: { fontSize: 8 }, headStyles: { fillColor: [50, 150, 80] },
    });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Analyse détaillée par produit, boutique, pays et vendeur</p>
        </div>
        <Button onClick={exportPDF} variant="outline" size="sm"><Download className="h-4 w-4 mr-2" /> Exporter PDF</Button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          ...
          <div><p className="text-[10px] text-muted-foreground uppercase tracking-wider">Chiffre d'affaires</p><p className="text-lg font-display font-bold">{formatCurrency(totalRevenue)}</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-blue-500/10 flex items-center justify-center"><ShoppingCart className="h-5 w-5 text-blue-500" /></div>
          <div><p className="text-[10px] text-muted-foreground uppercase tracking-wider">Volume de ventes</p><p className="text-lg font-display font-bold">{totalSalesCount}</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-green-500/10 flex items-center justify-center"><TrendingUp className="h-5 w-5 text-green-500" /></div>
          <div><p className="text-[10px] text-muted-foreground uppercase tracking-wider">Panier moyen</p><p className="text-lg font-display font-bold">{formatCurrency(avgSale)}</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-orange-500/10 flex items-center justify-center"><Package className="h-5 w-5 text-orange-500" /></div>
          <div><p className="text-[10px] text-muted-foreground uppercase tracking-wider">Produits vendus</p><p className="text-lg font-display font-bold">{filteredProducts?.reduce((s, p) => s + p.qty, 0) ?? 0}</p></div>
        </CardContent></Card>
      </div>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow>
              <TableHead>#</TableHead><TableHead>Produit</TableHead>
              <TableHead className="hidden md:table-cell">Boutique</TableHead><TableHead className="hidden md:table-cell">Pays</TableHead>
              <TableHead className="text-right">Qté</TableHead><TableHead className="text-right">CA</TableHead>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Vendeur</TableHead><TableHead className="text-right">Nb ventes</TableHead><TableHead className="text-right">CA</TableHead>
            </TableRow></TableHeader>
            <TableBody>
              {userData.length > 0 ? (
                userData.map((u, i) => (
